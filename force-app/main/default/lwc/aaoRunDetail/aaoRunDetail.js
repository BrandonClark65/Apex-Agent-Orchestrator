...
export default class AaoRunDetail extends NavigationMixin(LightningElement) {
  ...
  // A run with no version stamp predates prompt versioning, or ran against the packaged
  // baseline because the agent has no versions yet. Say so rather than showing a blank badge.
  get promptVersionLabel() {
    const number = this.detail?.summary?.promptVersionNumber;
    return number ? `prompt v${number}` : "baseline prompt";
  }
