...
- **No CORS.** The proxy serves the widget _and_ the API from the same origin, so the browser makes
...
| ---------------- | ------------------------------------ |
| **API base URL** | `/agent`                             |
| **Bearer token** | _(leave blank - the proxy holds it)_ |
| **External ref** | _(leave blank to auto-generate)_     |
